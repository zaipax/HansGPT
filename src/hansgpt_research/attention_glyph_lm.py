    ...
    def __init__(self, config: ModelConfig, variant: str, encoder=None, decoder=None):
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            if variant in {"B", "C"}:
                ...
                kind = decoder_config.pop("kind", "autoregressive")
                if kind not in {"autoregressive", "parallel"}:
                    raise ValueError("Unknown byte decoder kind")
                decoder_type = ParallelByteDecoder if kind == "parallel" else ConditionalByteDecoder
                self.byte_decoder = decoder_type(config.hidden_size, **decoder_config)
