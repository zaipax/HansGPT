...
def config(stop):
    return dict(lr_schedule='global_cosine',learning_rate=3e-4,minimum_learning_rate_ratio=0.1,
                schedule_total_positions=1089139385,warmup_positions=10891394,target_tokens=stop)
...
def test_global_warmup_reaches_peak_and_is_independent_of_gpu_count():
    ...
    assert position_learning_rate(1000000,dict(cfg,world_size=4))==position_learning_rate(1000000,dict(cfg,world_size=8))
