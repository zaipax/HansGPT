...
def install_xformers(*, opaque_backward=False):
    ...
    if opaque_backward:
        from hansgpt_research.compiled_cutlass_backward import cutlass_backward
    ...
    def backward_operator(*args, **kwargs):
        ...
        if opaque_backward:
            if (
                kwargs["bias"] is not None
                or kwargs["cu_seqlens_q"] is not None
                or kwargs["cu_seqlens_k"] is not None
                or kwargs["dropout_p"] != 0.0
                or kwargs["bias_requires_grad"]
            ):
                raise ValueError(
                    "Opaque backward requires fixed, bias-free, dropout-free attention"
                )
            grads = cutlass_backward(
                *args,
                kwargs["out"],
                kwargs["logsumexp"],
                kwargs["philox_seed"],
