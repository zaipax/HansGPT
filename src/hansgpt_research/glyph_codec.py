...
class SpatialQueryEncoder(nn.Module):
    ...
    def __init__(self, original, slots=4, width=256):
        ...
        self.queries=nn.Parameter(torch.randn(1,slots,inner)*.02)
        self.pool=nn.TransformerDecoderLayer(inner,4,inner*4,dropout=0,
    ...
    def forward(self,tiles):
        ...
        for layer in self.blocks:hidden=layer(hidden)
...
class GlyphCodec(nn.Module):
    ...
    def __init__(self,arm,encoder,decoder):
        super().__init__()
        if arm not in {'fixed','spatial'}:raise ValueError('Unknown codec arm')
        ...
        if arm=='fixed':
            self.encoder=copy.deepcopy(encoder).requires_grad_(False)
    ...
    def train(self,mode=True):
        super().train(mode)
        if self.arm=='fixed':self.encoder.eval()
        return self

    def encode(self,tiles):
        if tiles.ndim<4 or tiles.shape[-3:]!=(1,32,32):raise ValueError('Expected glyph bitmaps')
        leading=tiles.shape[:-3]
        flat=tiles.reshape(-1,1,32,32)
        validate_binary_tiles(flat.unsqueeze(1))
        if self.arm=='fixed':
            with torch.no_grad():features=self.encoder(flat)
            latents=self.adapter(features).reshape(-1,4,256)
        else:latents=self.encoder(flat)
        return latents.reshape(*leading,4,256)

    def decode(self,latents):
        if latents.shape[-2:]!=(4,256):raise ValueError('Expected [...,4,256] glyph latents')
