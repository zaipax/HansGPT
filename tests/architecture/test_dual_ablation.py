...
helpers = runpy.run_path(str(Path(__file__).resolve().parents[2] / "scripts/run_dual_ablation.py"))
...
def test_reconstruction_split_keeps_aliases_together_and_excludes_ineligible():
    ...
    for i in range(1, 5):
        bank[i, 0, 0, i] = 1
    bank[5] = bank[1]
    left, right = helpers["glyph_partition"](bank, [1, 2, 3, 4, 5], 519, 0.25)
    ...
    assert (left, right) == helpers["glyph_partition"](bank, [1, 2, 3, 4, 5], 519, 0.25)
